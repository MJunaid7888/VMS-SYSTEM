  ...
  try {
    ...
    if (status === "approved") {
      const qrPayload = {
        ...
        userType:
          doc.visitorCategory === "contractor" ? "contractor" : "visitor",
...
export const updateAdminConfig = async (req, res) => {
  try {
    const config = await AdminConfig.findOneAndUpdate({}, req.body, {
      upsert: true,
      new: true,
    });
    res.json(config);
...
export const exportVisitorsToExcel = async (req, res) => {
  try {
    ...
    if (
      ...
      visitors = await Visitor.find({ hostEmployeeId: user._id }).sort({
        createdAt: -1,
      });
    }
...
export const checkInVisitor = async (req, res) => {
  try {
    ...
    if (visitor) {
      ...
      return res
        .status(200)
        .json({ message: "Visitor checked in successfully", visitor });
    }
...
export const checkOutVisitor = async (req, res) => {
  try {
    ...
    if (visitor) {
      ...
      return res
        .status(200)
        .json({ message: "Visitor checked out successfully", visitor });
    }
    ...
    if (contractor) {
      ...
      return res.status(200).json({
        message: "Contractor checked out successfully",
        visitor: contractor,
      });
    }
      ...
      await sendEmail({
        ...
        html: `
          ...
          <ul>
            ...
            <li><strong>Start:</strong> ${new Date(
              visitor.visitStartDate
            ).toLocaleString()}</li>
            <li><strong>End:</strong> ${new Date(
...
export const validateQrCode = async (req, res) => {
  try {
    ...
    if (!qrData) {
      return res
        .status(400)
        .json({ success: false, message: "QR data not provided" });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid QR data format" });
    }
    ...
    if (visitorCategory === "visitor") {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Unknown role in QR data" });
    }
    ...
    if (person.status !== "approved") {
      return res
        .status(403)
        .json({ success: false, message: "Access denied: Not approved yet" });
    }
...
export const createTraining = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Missing required fields or questions." });
    }
...
export const updateSystemSettings = async (req, res) => {
  const updated = await SystemSetting.findOneAndUpdate({}, req.body.settings, {
    new: true,
    upsert: true,
  });
