...
export const getVisitorStats = async (req, res) => {
  try {
    ...
    const checkedInContractors = await Contractor.countDocuments({
      status: "checked-in",
    });
    const checkedOutContractors = await Contractor.countDocuments({
      status: "checked-out",
    });
    const scheduledContractors = await Contractor.countDocuments({
      status: "scheduled",
    });
    const pendingContractors = await Contractor.countDocuments({
      status: "pending",
    });
    const approvedContractors = await Contractor.countDocuments({
      status: "approved",
    });
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch visitor/contractor stats" });
  }
};
...
export const getAccessMetrics = async (req, res) => {
  try {
    ...
    const successfulAccesses = await AccessLog.countDocuments({
      status: "granted",
    });
...
export const getTrainingMetrics = async (req, res) => {
  try {
    ...
    const inProgress = await TrainingRecord.countDocuments({
      status: "in-progress",
    });
...
export const getVisitorMetrics = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to compute combined visitor metrics" });
  }
};
